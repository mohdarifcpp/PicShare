...
function Navbar() {
  ...
  return (
    <nav className='fixed top-0 w-full flex justify-between items-center bg-blue-800 h-12'>
      <Link to='/'>
        <figure className='flex items-center px-2 py-2 md:px-5 gap-1'>
          <img src={logo} className='w-5 md:w-6' alt='Picstagram' />
          <h1 className='tracking-wider text-lg md:text-xl uppercase font-extrabold mx-3 text-white font-mono'>
            PicShare
          </h1>
        </figure>
      </Link>
        ...
        {user?.uid ? (
          <div className='flex items-center md:gap-2'>
            ...
            <Link
              ...
              p-1 md:p-2 rounded-sm hover:bg-orange-200 hover:text-gray-800'>
              Sign Out
            </Link>
          </div>
        ) : (
          <Link
            className='uppercase px-3 py-2 rounded-sm hover:bg-orange-200 hover:text-gray-800'
            to='/signin'>
            Sign In
          </Link>
        )}
