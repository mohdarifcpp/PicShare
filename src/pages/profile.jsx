...
import { handleDelete } from "../components/photo/photo.component"
    ...
    <div className='mt-12 flex flex-col' key={userInfo?.imageUrl}>
      <figure className='mx-auto my-10'>
        <img
          ...
          alt={userInfo?.name}
        />
        <figcaption className='mx-auto text-black-400 uppercase'>
          <p className='text-center font-medium text-sm text-yellow-200'>{userInfo?.name}</p>
        </figcaption>
      </figure>
      <div
      ...
      sm:gap-6 md:gap-8 w-4/5 sm:w-3/4 md:w-11/12'>
        {imagesData?.map(item => (
          <div className='mx-auto p-2 bg-gray-300  rounded-lg   shadow-xl shadow-black'>
