...
function HomePage() {
  ...
  return (
    <div >
      ...
      <div
        className='mx-auto mt-20 grid grid-cols-1 xs:grid-cols-2 xs:gap-2 md:grid-cols-3 lg:grid-cols-5 
      ...
      {user?.uid ? (
        <div>
          <div className='fixed right-2 bottom-12  pr-4 border-solid border-slate-900'>
            <label
              className='rounded w-10 h-10 shadow-lg shadow-slate-300 cursor-pointer '
              htmlFor='image-upload'>
              <BsPlusLg size='2rem' className='text-white hover:text-red-700' />
            </label>
            <input
              id='image-upload'
              accept='image/*'
